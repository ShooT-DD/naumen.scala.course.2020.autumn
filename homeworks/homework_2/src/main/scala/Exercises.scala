...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        ...
        while(a != 1){
            ...
            n = n/a
            if(a == 1){
                if(!res.contains(n) && n != 1) {
                    res = res :+ n
                }
            }
            if(!res.contains(a) && a != 1) {
                res = res :+ a
            }
        }
        res
    ...
    */
    ...
    def WeightBall(a:(Int, Double)): Double = (4/3 * java.lang.Math.PI * Math.pow(a._1,3)) * a._2


    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.mapValues(i => WeightBall(i._1,i._2)).toList.sortBy(_._2).map(_._1)
    }
}